...
export default async function DashboardPage() {
  ...
  const manageCeremonies = can.manageCeremonies(wedding.role);
  ...
  const attention = (vendors ?? [])
    .map((v) => ({ ...v, pendingAmount: v.quoted_amount - (paidByVendor.get(v.id) ?? 0) }))
    .filter((v) => v.pendingAmount > 0)
    .sort((a, b) => b.pendingAmount - a.pendingAmount)
    .slice(0, 5);

  // Helpers get a task-focused view instead of the budget rollup.
  const ceremonyIds = (ceremonies ?? []).map((c) => c.id);
  const { data: openTasks } =
    !viewMoney && ceremonyIds.length > 0
      ? await supabase
          .from("tasks")
          ...
          .limit(6)
      : { data: [] as { id: string; description: string; due_at: string | null }[] };
  ...
  return (
    <div className="px-5 pt-[60px]">
      ...
      {viewMoney ? (
        ...
        <>
          <div className="mb-2.5 flex items-center justify-between">
            <span className="text-[15px] font-semibold text-ink">Your open tasks</span>
            <Link href="/tasks" className="text-[13px] text-accent">
              All tasks
            </Link>
          </div>
          <div className="flex flex-col gap-2.5">
